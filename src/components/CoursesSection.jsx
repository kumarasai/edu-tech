      ...
      <div className="mt-8">
        ...
        {!loading && error && (
          <div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-8 text-center text-destructive">
            ...
            <p className="mt-2 text-sm opacity-80">Please refresh the page and try again.</p>
          </div>
        )}
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="h-64 rounded-2xl border border-border bg-muted animate-pulse" />
      ))}
    </div>
  );
}
